...
const TabContent = ({data}) => {
    ...
    const initialFormData = {
        'blogCat': category.map(cat => ({ ...cat, readOnly: 'readonly' }))
    };
    ...
    const handleEditClick = async (id) => {
        ...
        const categoryToEdit = formData.blogCat.find(cat => cat.id === id);
        ...
        setFormData(prevState => ({
            ...prevState,
            blogCat: prevState.blogCat.map(cat => {
                if (cat.id === id) {
    ...
    const handleInputChange = (id, newValue) => {
        setFormData(prevState => ({
            ...prevState,
            blogCat: prevState.blogCat.map(cat => cat.id === id ? { ...cat, title: newValue, slug: slugify(newValue) } : cat)
        }));
    };
            ...
            <div className={`${styles['admin-page-wrap']}`}>
                ...
                <div className={`${styles['admin-cat-block']}`}>
                    ...
                    <div className={`${styles['admin-cats-list']}`}>
                        {formData.blogCat.map((item, index) => (
