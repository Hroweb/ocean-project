...
import {updateClients} from "@/utils/api/(admin)/post";
...
const TabContent = () => {
    ...
    const onSubmit = useCallback(async (formData) => {
        ...
        try {
            const response = await updateClients(formData);
            if (response.ok) {
        ...
        <>
            ...
            <div className={styles['admin-btn-row']}>
                <button type="submit" className="btn-primary" onClick={(e) => handleUniqueSubmit(e, onSubmit)} disabled={isSubmitting}>
                    {isSubmitting ? 'Processing...' : 'Add New Client'}
                </button>
            </div>
        </>
    )
